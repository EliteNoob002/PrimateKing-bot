...
async def _handle_upload_error(status: int, error_body: dict) -> None:
    ...
    if status in (401, 403):
        ...
        schedule_notify_api_panel_unreachable(
            "WaifuFiles upload", "upload_to_server", exc, config["webupload_base"],
        )
...
async def upload_to_server(file_path, user):
    ...
    base = config["webupload_base"].rstrip("/")
    url = f"{base}/api/v1/files/upload"
    upload_headers = {"X-API-Key": config["webupload_api_key"]}
    ...
    async with aiohttp.ClientSession() as session:
        with open(file_path, "rb") as file:
            ...
            data.add_field(
                "files", file, filename=original_filename, content_type="image/jpeg",
            )
            ...
            async with session.post(
                url, data=data, headers=upload_headers, timeout=timeout,
            ) as response:
                if response.status == 200:
